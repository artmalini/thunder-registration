...
//////////
//////     НИ НА ЧТО НЕ ВЛИЯЮЩАЯ СЕКЦИЯ ДЛЯ БЕЙДЖЕВ
//////////
////////
	...
	function get_verified_status($user_id) {
		$field = get_user_meta($user_id, 'thunder_verified', true);
		if (is_admin($user_id)) {
			//return 1;
			return $field;
		} else {
	...
	function admin_verify($user_id) {	
		...
		//do_action('thunder_after_user_verify', $user_id);
	}
	...
	function unverify($user_id) {
		...
		if ( thunder_get_option('notify_user_unverified') && get_verified_status($user_id) == 1 ){
			thunder_mail($user_id, 'accountunverified');
		}		
		// make user unverified and delete his request
		update_user_meta($user_id, 'thunder_verified', 0);
		delete_user_meta($user_id, 'thunder_verification');
		...
		if (isset($requests) && is_array($requests)){
			...
			update_option('thunder_verify_requests', $requests);
		}
		
		//do_action('thunder_after_user_unverify', $user_id);
	}

	///
	//Verify a user instantly
	//	
	add_action('wp_ajax_thunder_verify_user', 'thunder_verify_user');
	function thunder_verify_user(){
		if (!current_user_can('manage_options'))
			die(); // admin priv
		...
		$arr = get_option('thunder_verify_requests');
		if (isset($arr) && is_array($arr)){
			$arr = array_diff($arr, array( $user_id ));
			update_option('thunder_verify_requests', $arr);
		}
		
		$output['count'] = get_pending_verify_requests_count_only();
		
		//$output['admin_tpl'] = '<a href="#" class="button button-primary upadmin-unverify-u" data-user="'.$user_id.'">'.thunder_get_badge('verified') . '</a>';
		$output['admin_tpl'] = '<a href="#" class="button button-primary upadmin-unverify-u" data-user="'.$user_id.'">verified</a>';
		if (get_verified_status($user_id) == 0){
			$output['admin_tpl'] .= '<a href="#" class="button upadmin-invite-u" data-user="'.$user_id.'">'.__('Verified Invite','thunder') . '</a>';
		}
		
		//do_action('thunder_after_account_verified');
	...
	///	
	add_action('wp_ajax_thunder_unverify_user', 'thunder_unverify_user');
	function thunder_unverify_user(){
		if (!current_user_can('manage_options'))
			die(); // admin priv
		...
		$output['admin_tpl'] = '<a href="#" class="button upadmin-verify-u" data-user="'.$user_id.'">nverified</a>';
		if (get_verified_status($user_id) == 0){
			$output['admin_tpl'] .= '<a href="#" class="button upadmin-invite-u" data-user="'.$user_id.'">'.__('Verified Invite','thunder') . '</a>';
		}
		
		//do_action('thunder_after_account_unverified');
