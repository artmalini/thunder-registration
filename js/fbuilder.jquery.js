...
(function($) {
	$.fn.fbuilder = function(options){
		...
		{
		  for (var i=0;i<opt.typeList.length;i++)		  			
			  $("#tabs-1").append('<div class="button width40 '+"itemForm"+'" id="'+opt.typeList[i].id+'">'+opt.typeList[i].name+'</div>');
  ...
  $('.load_avatar').on('click', function() {
      $.uploadPreview({
      input_field: "#image-upload",   // Default: .image-upload
      preview_box: ".prev_avatar",  // Default: .image-preview
      ...
      no_label: false                 // Default: false
				...
				//jQuery(this).hide().append("<input type='file' name='thunder_file_avatar'/>")
...
/*			var fcheck=function(){};
		$.extend(fcheck.prototype,ffields.prototype,{
				...
				display:function(){
					...
					for (var i=0;i<this.choices.length;i++)
						str += '<div class="'+this.layout+'">
						<input class="field" disabled="true" type="checkbox" '+((this.choiceSelected[i])?"checked":"")+'/> '+this.choices[i]+'</div>';
					
					return '<div class="fields" id="field'+opt.identifier+'-'+this.index+'">
					<div class="arrow ui-icon ui-icon-play ">
					</div><div class="remove ui-icon ui-icon-trash "></div>
					<label>'+this.title+''+((this.required)?"*":"")+'</label>
					<div class="dfield">'+str+'<span class="uh">'+this.userhelp+'</span></div>
				...
				showChoiceIntance: function() {
					...
					for (var i=0;i<l.length;i++)
					{
						...
						<input class="choice_check" i="'+i+'" type="checkbox"
						 '+((this.choiceSelected[i])?"checked":"")+'/>

						 <input class="choice_text" i="'+i+'
						 " type="text" name="sChoice'+this.name+'" id="sChoice'+this.name+'" value="'
						 +htmlEncode(l[i])+'"/>

						 <input class="choice_value" i="'+i+'" type="text" name="sChoice'
						 +this.name+'V'+i+'" id="sChoice'+this.name+'V'+i+'" value="'+htmlEncode(lv[i])+'"/>
						 
						 <a class="choice_add ui-icon ui-icon-circle-plus" i="'+i+'" title="Add another choice.">
						 </a><a class="choice_remove ui-icon ui-icon-circle-minus" i="'+i+'" title="Delete this choice."></a></div>';
		...
		var favatar=function(){};
		$.extend(favatar.prototype,ffields.prototype,{
				title:"Profile avatar",
				ftype:"favatar",
				name: "thunder_file_avatar",
				predefined:"",
				predefinedClick:false,
				required:false,				
				display:function(){
					var str = "";
					str += '<div class="fields" id="field'+opt.identifier+'-'+this.index+'">';
					str += '<div class="arrow ui-icon ui-icon-play "></div>';
					str += '<div class="remove ui-icon ui-icon-trash "></div>';
					str +=  '<label>'+this.title+''+((this.required)?"*":"")+'</label>';
					str += '<div class="dfield">';
					str += '<div class="prev_avatar"><img src="http://lol.com" alt="xD" class="default" width="64" height="64" /></div>'
					str += '<div class="load_avatar"><label for="image-upload" id="image-label">Choose File</label><input type="file" name="thunder_file_avatar" id="image-upload" /></div>'
					//str += '<input class="field disabled '+this.size+'" type="file" name="' + favatar.prototype.name + '" value="'+htmlEncode(this.predefined)+'"/>';					
					str += '<span class="uh">'+this.userhelp+'</span></div><div class="clearer"></div></div';
					return str;
				},
				show:function(){
					return '<div class="fields '+this.csslayout+'" id="field'+opt.identifier+'-'+this.index+'"><label>'+this.title+''+((this.required)?"*":"")+'</label><div class="dfield"><input id="'+this.name+'" name="'+this.name+'"class="field '+this.size+((this.required)?" required":"")+'" type="file" value="'+htmlEncode(this.predefined)+'"/><span class="uh">'+this.userhelp+'</span></div><div class="clearer"></div></div>';
				},
                showSpecialDataInstance: function() {
                	var str = "";
                	str += '<div class="column">';
                	str += '<label>Min length/characters</label><br />';
                	str += '<input name="sMinlength" id="sMinlength" value="'+this.minlength+'"></div>';
                	str += '<div class="column">';
                	str += '<label>Max length/characters</label><br />';
                	str += '<input name="sMaxlength" id="sMaxlength" value="'+this.maxlength+'"></div>';
                	str += '<div class="clearer"></div>';                	
                    return str;
                }
		});
		var fremember=function(){};
		$.extend(fremember.prototype,ffields.prototype,{
				title:"Remember Me",
				ftype:"fremember",
				predefined:"forever",
				predefinedClick:false,
				...
				display:function(){
					...
					str +=  '<label>'+this.title+''+((this.required)?"*":"")+'</label>';
					str += '<div class="dfield">';
					str += '<input class="field disabled '+this.size+'" type="checkbox" value="'+htmlEncode(this.predefined)+'"/>';
					str += '<span class="uh">'+this.userhelp+'</span></div><div class="clearer"></div></div';
					return str;
				},
				show:function(){
					return '<div class="fields '+this.csslayout+'" id="field'+opt.identifier+'-'+this.index+'"><label>'+this.title+''+((this.required)?"*":"")+'</label><div class="dfield"><input id="'+this.name+'" name="'+this.name+'"class="field '+this.size+((this.required)?" required":"")+'" type="checkbox" value="'+htmlEncode(this.predefined)+'"/><span class="uh">'+this.userhelp+'</span></div><div class="clearer"></div></div>';
				},
                showSpecialDataInstance: function() {
                	var str = "";
                	str += '<div class="column">';
	   ...
	   loadtmp = function(p)
	   {
		   ...
		   if ( d = $.parseJSON(p))
		   {
			   if (d.length==2) //array length 2 here
			   {
				   items = new Array();
				   for (var i=0;i<d[0].length;i++)
				   {
					   ...
					   obj.name = obj.name;//+opt.identifier;   //to the string
					   items[items.length] = obj; //push first into items
				   }
				   theForm = new fform();
