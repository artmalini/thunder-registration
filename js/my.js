...
var fire = (('ontouchstart' in window) || (window.DocumentTouch && document instanceof DocumentTouch)) ? 'touchstart' : 'click';

/* jQuery("a,input").live('click',function(){
        if (jQuery(this).data('tpl')) {
            container = jQuery(this).parents('.thunder');
            form_data = jQuery(this).data();     //register  
            shortcode = '[thunder';
            ...
            thunder_secondary_template( 'update', jQuery(this).parents('.thunder'), shortcode );
        }
     })

 function thunder_secondary_template( method, container, shortcode ) {
        ...
        jQuery.ajax({
            ...
            success:function(data){
                ...
                console.log(jQuery(this)); 
                    ...
                    var item = jQuery(this).parents('.thunder');
                    thunder_picupload();          
            }
        });
...
jQuery( document ).on( fire, 'input.fields-trigger', function(e) {       
    console.log('thunder-secondary');
            // form data and shortcode
        if (jQuery(this).data('tpl')) {
            container = jQuery(this).parents('.thunder');
...
function thunder_collapse(){
    ...
    jQuery('.thunder-collapsible-1.thunder-collapsed-1').each(function(){
        ...
        if (jQuery(this).find('.thunder-icon.angle-right').length==0) jQuery(this).prepend('<span class="thunder-icon angle-right"></span>');
    });
    jQuery('.thunder-collapsible-1.thunder-collapsed-0').each(function(){
        jQuery(this).nextUntil('.thunder-collapsible-1').show();
        if (jQuery(this).find('.thunder-icon.angle-down').length==0) jQuery(this).prepend('<span class="thunder-icon angle-down"></span>');
    });
}
...
jQuery( document ).on('click', '.thunder-collapsible-1', function(){
    ...
    if (jQuery(this).nextUntil('.thunder-collapsible-1').is(':hidden')){
        ...
        //jQuery(this).find('.thunder-icon.angle-down').html('<i class="thunder-icon angle-down"></i>');
        jQuery(this).find('.thunder-icon.angle-right').removeClass('angle-right').addClass('angle-down');
    
    
    } else {
        jQuery(this).nextUntil('.thunder-collapsible-1').hide();
        //jQuery(this).find('.thunder-icon.angle-right').html('<i class="thunder-icon angle-right"></i>');
        jQuery(this).removeClass('thunder-collapsed-0').addClass('thunder-collapsed-1');
        jQuery(this).find('.thunder-icon.angle-down').removeClass('angle-down').addClass('angle-right');
    }
      ...
      // var ajaxcheck = element.data('ajaxcheck');
        ...
        if (required == 1) {
            ...
            if ( element.attr('type') == 'password') {
                ...
                /*if ( element.val().replace(/^\s+|\s+$/g, "").length == 0) {
                    thunder_client_error( element, parent, element.parents('.thunder').data('required_text') );
                } else if ( element.val().length < 8 ) {
                    thunder_client_error( element, parent, element.parents('.thunder').data('password_too_short') );
                } else if ( thunder_password_strength_meter( element ) < 3 ) {
                    thunder_client_error( element, parent, element.parents('.thunder').data('password_not_strong') );
                } else {
                    thunder_client_valid(element, parent);
                }  */              
            }
...
//var flag = false;
    jQuery( document ).on( fire, '.thunder-login-sub', function(e) { 
    /*if (!flag) {
    flag = true;
    setTimeout(function(){ flag = false; }, 100); */ 
       ...
       form.find('select').each(function() {
        ...
        if ( check.length == 0 ) {
            ...
            var templ = jQuery(this).closest('form').data('tpl'); // thunder-warning error
            ...
            jQuery.ajax({
                ...
                success:function(data){
                    ...
                    if (data.error){ 
                        ...
                        jQuery.each( data.error, function(key, value) {
                            ...
                            parent.find('.warning-ok').remove(); // remove previous required valid check
                            if (element){
                    ...
                    if (data.custom_message){
                        form.find('.warning-ok').remove(); // remove previous required valid check
                        ...
                        jQuery('.thunder-message').fadeOut(9000);
                    }
                ...
                if (data.tpl){
                  ...
                  /* thunder_responsive();
                    thunder_chosen();
                    thunder_fluid_videos();
                    thunder_ajax_picupload();
                    thunder_collapse();
                    thunder_overlay_center('.thunder-overlay-inner');*/
            ...
            jQuery('.thunder-collapsible-1').each(function(){
                jQuery(this).nextUntil('.thunder-collapsible-1').show();
                if (jQuery(this).find('thunder-icon.angle-right').length > 0 ) {
                    jQuery(this).find('thunder-icon.angle-right').remove();
                    jQuery(this).prepend('<span class="thunder-icon angle-down"></span>');
                }
            });
            return false;
        }
...
function thunder_client_error( element, parent, error) {
    //if ( element.attr('type') )   
        ...
        if (parent.find('.thunder-warning').length==0) {
            ...
            parent.find('.thunder-warning').animate({'top' : '0px', 'opacity' : '1'});
        }     
}
...
//jQuery('.thunder-pic-upload').each()

function thunder_picupload() {
    //console.log(this);
    jQuery( '.thunder-pic-upload' ).each( function() {
        var allowed = jQuery(this).data('allowed_extensions');
        var filetype = jQuery(this).data('filetype');
    
        var item = jQuery( this ).closest('.thunder-input');
        /*if ( item.find('input:hidden').val() != '' ) {
          item.find('.thunder-button.remove').trigger( 'init:remove' );
        }*/
   ...
   // console.log(this);
    ...
    jQuery(this).uploadFile({
        ...
        onSubmit:function(files){            
            ...
            statusbar.parents('.thunder-input').find('.remove').hide();
            //statusbar.find('.ajax-file-upload-red').show();
        ...
        onSuccess:function(files,data,xhr){ 
            if (data != '') {
	            ...
	            var statusbar = item.find('.ajax-file-upload-statusbar:visible');
	            var src = data.target_file_uri;
	                      
	            str = 'action=thunder_crop_picupload&filetype='+filetype+'&src='+src;
	            jQuery.ajax({
	                url: my_data.ajaxurl,
	                data: str,
	                dataType: 'JSON',
	                ...
	                success:function(data){
	                    console.log(data);
	                    statusbar.prev().after("<input type='button' value='" + statusbar.parents('.thunder-input').find('.thunder-pic').data('remove_text') + "' class='thunder-button remove' style='display:none' />");
	                    statusbar.prev().fadeIn( function() {
	                        if (filetype == 'ava_picture'){                         
	                        statusbar.parents('.thunder-input').find('img').attr('src', data.response );
	                        statusbar.parents('.thunder-input').find('input:hidden').val( data.response );
	                        } else if (filetype == 'file'){
	                            statusbar.parents('.thunder-input').find('.thunder-file-input').remove();
	                            statusbar.parents('.thunder-input').prepend( data.response );
	                            statusbar.parents('.thunder-input').find('input:hidden').val( src );
	                        }
	                        statusbar.hide();
	                    });	                    
	                    statusbar.parents('.thunder-input').find('.thunder-pic-none').hide();
	                }
	            });            	
            } else {
            	//console.log('cant load');
            	item.find('.ajax-file-upload-statusbar:visible').hide();
            	var fileitem = item.find('.thunder-file-input a');
            	if (fileitem.length > 0) {
            		fileitem.remove();
            	}
            	item.append('<div class="file-err-msg">file can\'t be uploaded</p>')
            	item.find('.file-err-msg').fadeOut(5000);
            }
...
/*jQuery( document ).on( fire, '.thunder-pic-upload', function(){
    var allowed = jQuery(this).data('allowed_extensions');
    var filetype = jQuery(this).data('filetype');
    ...
    jQuery( document ).on( fire, '.thunder-button.remove', function() {  
        console.log('yep');      
        jQuery( this ).trigger( 'init:remove' );                
    });

jQuery( document ).on('init:remove', '.thunder-button.remove', function(e){
    var file = jQuery(this).closest('.thunder-input');
    var key = file.find('.thunder-pic-upload').data('filetype');
    var thumb = file.find('.thunder-avathumb');
    var filename;
    if (thumb.val() != undefined) {
        thumb = '' + thumb.val().replace(/(.+\/bfi_thumb\/?.?\/)(?=.?)/gi, '' );
        filename = thumb.replace(/(-.+)(?=\.)/gi, '' );
    }     

    var uploadfiles = file.find('.thunder-file-input a').text();
   
    var item= jQuery(e.target).closest(' .thunder-input ');
    var url = item.find('.thunder-pic').data('ava');
    var size = item.find('.thunder-pic').data('ava_size');

    if (key == 'ava_picture') {
        str = 'action=remove_file&key='+key+'&thumb='+thumb+'&filename='+filename;
    } else {
        str = 'action=remove_file&key='+key+'&filename='+uploadfiles;
    }
    ...
    jQuery.ajax({
        ...
        success:function(data){
            console.log(data);
            //'<img src="'.$url.'" width="'.$args['size'].'" height="'.$args['size'].'" alt="'.$args['alt'].'" class="default avatar" />';


            if (key == 'ava_picture') {                
                //console.log(avatar);                
                item.find('.thunder-pic').find('img').remove();//.append('<img src="'+url+'" width="'+size+'" height="'+size+'" alt="default-avatar" class="default avatar" />');
                item.find('.thunder-pic').append('<img src="'+url+'" width="'+size+'" height="'+size+'" alt="default-avatar" class="default avatar" />');
                item.find('.ajax-file-upload-statusbar').remove();
                item.find('.remove').remove();
                item.find('.thunder-avathumb').val('');
            } else {
                console.log(e);
                item.find('.thunder-file-input').remove();
                item.find('.thunder-pic span').show();
                item.find('.ajax-file-upload-statusbar').remove();
                item.find('.remove').remove();                
                item.find('.thunder-filethumb').val('');
            }
